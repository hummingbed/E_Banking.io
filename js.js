...
infoname = ["michael","Menebraye","Oyinbrakemi","Solomon"];
balance = [0,0,0,0];
typemoney =["NGN","NGN","NGN","USD"];
rank = ["Savings","Current","Domiciliary","Fixed Deposit"];
joindate = ["22/03/2021","10/02/2020","22/03/2021","10/02/2020"];
country = ["Nigeria", "United State", "Ghana","canada"]
...
function Deposit() {
    ...
    document.getElementById("balance").innerHTML =prbalance;  
    document.getElementById("notification") .innerHTML= "Success. You deposit "+input+" NGN on balance";
    }
}
...
function Withdrawal() {
	...
	if (input<0) {
        ...
        document.getElementById("notification").innerHTML = "Success withdrawal " + input + " NGN";
        prbalance=account[success];
        document.getElementById("balance").innerHTML =prbalance;
    	...
    	document.getElementById("notick").style.display = 'none';
    	document.getElementById("errortick").style.display = 'block';
        document.getElementById("notification").innerHTML = "Error. Your balance is " + account[success] + " NGN. You need more 1 NGN to request";
    }
}
...
function Transfers(){
	...
	if ((input == id[i] && inputmoney>0 && account[success]-inputmoney>0)){
		...
		document.getElementById("errortick").style.display = 'none';
		document.getElementById("balance").innerHTML = prbalance;
		document.getElementById("notification") .innerHTML= "Success. You have send the money "+inputmoney+" NGN in account "+id[i];
	}else{
		document.getElementById("errortick").style.display = 'block';
    	document.getElementById("notick").style.display = 'none';
	}
